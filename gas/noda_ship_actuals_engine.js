...
/* 読み直したときの数量と検算を決める（純関数）。
   ★ 本文の「◯本」も読むたびに取れたり取れなかったりする。取れなかった回に
     範囲から数えた本数で数量を上書きすると、直したかったのは範囲だけなのに
     出荷本数まで変わる（524行目 26-60602 で 2本→1本 になった）。
     本文の数量が取れなかったときは、今シートにある数量を使う。 */
function shipact_recheckQty_(oldQty, f) {
  var q = (f && f.qty != null) ? f.qty
        : (oldQty === '' || oldQty == null || isNaN(Number(oldQty)) ? null : Number(oldQty));
  return shipact_pickQuantity_(q, f ? f.rangeQty : null);
}
...
/* ===== 人がPDFを見て確かめた容器番号 =====
   ★ PDFの読み取りは実行のたびに結果が変わる。人が確かめた値は、
     どの読み直しでも上書きしない。ここに載っている依頼Noの行は読み直しから外す。
   ★ 増やすときは、PDFを目で見て確かめた値だけを書くこと。 */
var SHIPACT_HUMAN_CONFIRMED = {
  // 読むたびに 27253 と 27263 で揺れた。2026-09-23 落合さんがPDFで 27253 と確認
  '60612': { 開始: 27253, 終了: 27253 }
};

/* 人が確かめた依頼Noか（純関数）。年度の有無は問わない。 */
function shipact_isHumanConfirmed_(依頼No) {
  return !!SHIPACT_HUMAN_CONFIRMED[String(依頼No || '').replace(/^\d{2}-/, '')];
}
...
function 容器Noの範囲を読み直す(依頼Noの一覧) {
  ...
  try {
    ...
    for (; i < values.length; i++) {
      ...
      var r = values[i];
      var chk = String(r[H['検算']] || '');
      // ★ 人が確かめた行は読み直さない（名指しされても）
      if (shipact_isHumanConfirmed_(r[H['依頼No']])) continue;
      if (only) {
      ...
      var picked = shipact_recheckQty_(r[H['数量']], f);
...
// ★ 「緩い判定で直した行を見直す」「組の両方を外した行を戻す」は役目を終えたので消した。
//   どちらも名指しで1回読んで書く関数で、もう一度実行すると読みの揺れで値を動かしてしまう。
...
function 本数の少ない直しを確かめる(揃える) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    if (!want[no]) continue;
    if (shipact_isHumanConfirmed_(no)) {
      Logger.log((i + 2) + '行目 ' + r[H['依頼No']] + '  人がPDFで確かめた値なので読み直さない（' +
                 r[H['容器No開始']] + '〜' + r[H['容器No終了']] + '）');
      continue;
    }
    var fileId = String(r[H['fileId']] || '');
    if (!fileId) continue;
    ...
    if (揃える === true && v.書く) {
      var f0 = parsed[0];
      var picked = shipact_recheckQty_(r[H['数量']], f0);
...
// ★ 名前を「書き換える」で始める。見るだけの「本数の少ない直しを確かめる」と
//   先頭が同じだと、関数の一覧で取り違える（実際に取り違えた）。
function 書き換える_化けた読みを戻す() {
