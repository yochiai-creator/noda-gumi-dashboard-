...
/* ★ 出荷は人が押さない。指図書の容器Noと突き合わせて自動で引く。
     押し忘れで数字がずれるのを防ぐため（落合さんの指示）。 */
const LOT_TONE = {
  未受検: { bg: "#fef3c7", fg: "#b45309" },
  受検済: { bg: "#dbeafe", fg: "#1d4ed8" },
  入庫済: { bg: "#dcfce7", fg: "#15803d" },
...
function ProdLotCard({ data, types, locations, onAdd, onInspect, onStockIn, onCancel, saving }) {
  ...
  const today = (() => {
    const d = new Date(), p = (n) => (n < 10 ? "0" + n : String(n));
  ...
  const count = (() => {
    if (!form) return null;
    const a = Number(form.開始), b = Number(form.終了);
    if (!form.開始 || !form.終了 || isNaN(a) || isNaN(b) || b < a) return null;
    return b - a + 1;
  })();

  const shown = filter ? lots.filter((l) => l.状態 === filter) : lots;

  return (
    <div>
      {/* 状態ごとの本数。ここが「流れ」の全体像 */}
      <div className="flex flex-wrap gap-1.5 mb-2">
        {[["未受検", t && t.未受検], ["受検済", t && t.受検済], ["入庫済", t && t.入庫済],
          ["出荷済", t && t.出荷済今月]].map(([k, v]) => (
          <button key={k} onClick={() => setFilter(filter === k ? "" : k)}
            className="px-2 py-1 rounded-md text-left"
            style={{ background: filter === k ? LOT_TONE[k].fg : LOT_TONE[k].bg, border: "none" }}>
            <span className="block text-[10px]"
              style={{ color: filter === k ? "#fff" : LOT_TONE[k].fg }}>
              {k === "出荷済" ? "今月 出荷済" : k === "受検済" ? "受検済・未入庫" : k}
            </span>
            <span className="block text-[15px] font-bold tabular-nums"
              style={{ color: filter === k ? "#fff" : LOT_TONE[k].fg }}>
              {v == null ? "—" : vizComma(v)}<span className="text-[10px] font-normal"> 本</span>
            </span>
          </button>
        ))}
      </div>
      ...
      {!adding ? (
        <button onClick={openAdd}
          className="px-3 py-1.5 rounded-md text-xs font-semibold text-white mb-2"
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50 p-2 mb-2">
          ...
          <span className="flex items-center gap-1.5 mb-1.5">
            ...
            <input value={form.接頭辞} placeholder="HEP" inputMode="text"
              onChange={(e) => setForm({ ...form, 接頭辞: e.target.value })}
              className="w-14 px-2 py-1 rounded-md border border-slate-200 text-sm" />
            <input value={form.開始} placeholder="54401" inputMode="numeric"
              onChange={(e) => setForm({ ...form, 開始: e.target.value })}
              className="w-20 px-2 py-1 rounded-md border border-slate-200 text-sm tabular-nums" />
            <span className="text-[11px]" style={{ color: VIZ.muted }}>〜</span>
            <input value={form.終了} placeholder="54480" inputMode="numeric"
              onChange={(e) => setForm({ ...form, 終了: e.target.value })}
              className="w-20 px-2 py-1 rounded-md border border-slate-200 text-sm tabular-nums" />
          </span>
          {/* ★ 本数は番号から数える。人に数えさせると必ずずれる */}
      ...
      {data && data.error && (
        <p className="text-xs text-amber-700 py-1">{data.error}</p>
      )}
      {lots.length === 0 ? (
        <p className="text-xs text-slate-500 py-2">まだ登録がありません。</p>
      ) : shown.length === 0 ? (
        <p className="text-xs text-slate-500 py-2">「{filter}」のロットはありません。</p>
      ) : (
        <div className="divide-y divide-slate-100 rounded-md border border-slate-200">
          {shown.slice(0, 40).map((l) => {
            const tone = LOT_TONE[l.状態] || LOT_TONE.未受検;
            const nokori = l.本数 - l.出荷済本数;
            return (
              <div key={l.ロットID} className="px-3 py-2">
                <span className="flex items-baseline gap-2">
                ...
                <span className="block text-[10px]" style={{ color: VIZ.muted }}>
                  {l.置場名 ? "置場 " + l.置場名 + "（" + l.置場番号 + "）" : ""}
                  {l.依頼No ? "　指図書 " + l.依頼No : ""}
                  {l.出荷日 ? "　" + l.出荷日 + " 出荷" : ""}
                </span>

                {l.状態 === "未受検" && (
                  <button disabled={saving} onClick={() => onInspect(l.ロットID)}
                  ...
                  pickFor === l.ロットID ? (
                    <span className="block mt-1">
                      ...
                      <span className="flex flex-wrap gap-1">
                        {locations.map((loc) => (
                          <button key={loc.no} disabled={saving}
                            onClick={() => onStockIn(l.ロットID, loc.no, () => setPickFor(null))}
                            className="px-2 py-1 rounded-md text-[11px] bg-slate-100"
                    ...
                    <button onClick={() => setPickFor(l.ロットID)}
                      className="mt-1 px-3 py-1 rounded-md text-[11px] font-semibold text-white"
                      style={{ background: NAVY }}>入庫する</button>
                  )
                )}
                {l.状態 === "入庫済" && (
...
function YardMap({ yardLive, onRefresh }) {
  ...
  /* ★ 毎回の描画で作り直さない。指でマップを動かしているあいだも描画は走るので、
       ここで区画を全部組み直すと、その回数だけ無駄が乗る。 */
  const data = React.useMemo(
    () => yard_mergeLiveBlocks_(rawData, liveForSize), [rawData, liveForSize]);
  ...
  /* ★ 指を動かしているあいだは live に入れて、SVGのviewBoxを直に書き換える。
       setZoom を毎回呼ぶと、そのたびにマップ全体（区画・一覧・凡例）が
       React で描き直されて指に付いてこない。指を離したときに1回だけ state に移す。
       実測（CPUを6分の1に落として計測）で 1コマ37msかかっていたのが17msになった。 */
  const gest = React.useRef({ mode: null, moved: 0, px: 0, py: 0, startZoom: null,
                              startDist: 0, suppressClick: false, live: null,
                              raf: 0, pending: null });
  ...
  const unitPerPx = (scale) => {
    const el = svgRef.current;
    const rect = el ? el.getBoundingClientRect() : null;
    const s = scale || curScale;
    if (!rect || !rect.width) return baseW / 360;
    return (baseW / s) / rect.width;
  };
  /* ★ 指で動かしているあいだは区画の文字を消す。
       実測（CPUを6分の1に落として計測）で、20kのマップは1コマ76msかかるが、
       文字を外すと17msまで落ちた。重いのは区画の枠ではなく文字のほう。
       動かしている最中の文字はどうせ読めないので、指を離したら戻す。 */
  const setDragging = (on) => {
    const el = svgRef.current;
    if (!el) return;
    if (on) el.classList.add("ymap-drag");
    else el.classList.remove("ymap-drag");
  };
  /* ジェスチャ中だけ使う。Reactを通さずSVGの表示範囲を直に書き換える。
     ★ 1コマに1回しか書かない。touchmove は1コマに何度も飛んでくることがあり、
       そのたびに書くと、同じ絵を何度も描き直すだけで重くなる。 */
  const applyViewBox = (z) => {
    ...
    g.raf = requestAnimationFrame(() => {
      ...
      el.setAttribute("viewBox",
        v.x + " " + v.y + " " + (baseW / v.scale) + " " + (baseH / v.scale));
    });
  };
  ...
  const onTouchStart = (e) => {
    const t = e.touches;
    const z = gest.current.live || zoom || { x: baseX, y: baseY, scale: 1 };
    /* ★ 前のジェスチャで立てた「タップ扱いにしない」印をここで落とす。
         指で動かしたあとは click が飛んでこないので、印が残ったままになり、
         次の本物のタップが1回分食われていた（2回タップしないと区画が開かない）。 */
  ...
  const onTouchMove = (e) => {
    ...
    if (g.mode === "pinch" && t.length === 2 && g.startDist > 0) {
      ...
      if (rect && rect.width) {
        ...
        g.live = clampZoom(nx, ny, nextScale);
      } else {
        g.live = clampZoom(g.startZoom.x, g.startZoom.y, nextScale);
      }
      applyViewBox(g.live);
      ...
      g.px = t[0].clientX;
      g.py = t[0].clientY;
      const z0 = g.live || zoom;
      if (z0 && z0.scale > MIN_SCALE) {   // 等倍のときは動かさない（区画のタップを邪魔しないため）
        const u = unitPerPx(z0.scale);
        g.live = clampZoom(z0.x - dx * u, z0.y - dy * u, z0.scale);
        applyViewBox(g.live);
      }
    }
  };
  ...
  const onTouchEnd = (e) => {
    ...
    // ★ 指が全部離れたときに1回だけ state に移す。1本残っているなら続きがある
    if (e && e.touches && e.touches.length > 0) return;
    if (gest.current.raf) { cancelAnimationFrame(gest.current.raf); gest.current.raf = 0; }
    gest.current.pending = null;
    setDragging(false);
    const live = gest.current.live;
    gest.current.live = null;
    if (!live) return;
    setZoom(live.scale <= MIN_SCALE ? null : live);
  };
  ...
  const resetZoom = () => { gest.current.live = null; setZoom(null); };
  ...
  const switchSize = (s) => {
    gest.current.live = null;
    setSize(s); setSel(null); setEditing(false); setZoom(null);
  };
...
function DispatchGrid({ grid, onSave, onWeek, saving }) {
  ...
  /* ★ 行き先が長いとマスが縦に伸びて行の高さがバラバラになる
       （「東京都西多摩郡瑞穂町 東京都羽村市」で3行になっていた）。
       週表示は1行で打ち切って高さをそろえる。全文は「日」表示で読める。 */
  const clamp1 = {
...
export default function App() {
  ...
  /* 生産の流れ（登録・受検・入庫）。
     ★ どれも保存できたらサーバから取り直す。入庫は置場の実績数も動かすので、
       置場の一覧とKPIも一緒に取り直す（画面だけ書き換えると食い違う）。 */
  const runLotAction = (invoke, done) => {
    if (!isGasEnv) return;
    setYardSaving(true);
    /* ★ ハンドラを付けてから呼ぶ。google.script.run は呼ぶたびに別の
         入れものを返すので、先にメソッドを呼ぶと結果を受け取れない。 */
    invoke(google.script.run
      .withSuccessHandler((r) => {
        setYardSaving(false);
        if (r && r.ok === false) { window.alert(r.error || "できませんでした"); return; }
        if (done) done();
        fetchYardTab();
        google.script.run
  ...
  /* 今月の実績（アームの月別出荷から）。予定ではなく、今日までに出た台数。
     ★ 月別出荷はサーバ側で今日までに切ってあるので、今月の行をそのまま使える。 */
  const armToday = (live.armMonthly && live.armMonthly.today) || null;
  const armCur = (() => {
    const ms = (live.armMonthly && live.armMonthly.months) || [];
    if (!armToday) return null;
    const ym = armToday.slice(0, 7);
  ...
  const armRange = (() => {
    ...
    return vizMonthLabel(ms[0].年月) + "〜" + vizMonthLabel(ms[ms.length - 1].年月);
  })();
  ...
  const tabKpis = {
    ...
    actuals: (() => {
      ...
      return [
        ...
        /* ★ アームは容器と単位が違う（台）。同じ並びに置くが、単位を「台」に
               しておけば取り違えない。中身は実績だけ（先の予定は入っていない）。 */
        { label: "アーム 累計" + (armRange ? "（" + armRange + "）" : ""),
          value: live.armMonthly && live.armMonthly.total != null
            ? vizComma(live.armMonthly.total) : "—",
          unit: "台", icon: Truck, tone: "neutral" },
      ];
    })(),
    dispatch: [
