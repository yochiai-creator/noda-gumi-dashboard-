...
export default function App() {
  ...
  /* 前月の実績（アームの月別出荷から）。予定ではなく済んだ月の台数。 */
  const armPrev = (live.armMonthly && live.armMonthly.prev) || null;
  const armPrevLabel = armPrev ? vizMonthLabel(armPrev.年月) : "前月";
  const armPrev13 = armPrev && armPrev.区分別 ? (armPrev.区分別["13ton"] || 0) : null;
  ...
  const tabKpis = {
    ...
    arm: [
      ...
      { label: armPrevLabel + " 出荷", value: armPrev ? String(armPrev.台数) : "—",
        unit: "台", icon: Package, tone: "ok" },
      { label: armPrevLabel + " うち 13ton", value: armPrev13 == null ? "—" : String(armPrev13),
