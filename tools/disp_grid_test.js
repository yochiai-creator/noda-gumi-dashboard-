...
(async () => {
  ...
  /* モックのトラックは9台。週まるごと空なのは「4ｔ平標準 ③」（自社）と
     「傭車1」「傭車2」（山陰急送 傭車便）の3台。
     ★ 傭車はスポットなので空き台数に数えない → 1台。
     運休（×）やお休みは「予定が入っている」扱い（マスが埋まっているため）。 */
  chk('空きトラックの欄が出ている', idle !== null, idle);
  chk('★傭車を除いた空き台数が出る（1台）', idle && /この週の空きトラック\s*1\s*台/.test(idle), idle);
  chk('★傭車を数えていないことが分かる（傭車2台は数えず）',
    idle && /傭車2台は数えず/.test(idle), idle);
  chk('予定ありの台数が出る（6台）', idle && /予定あり\s*6台/.test(idle), idle);
  chk('全台数が出る（全9台）', idle && /全9台/.test(idle), idle);
