...
console.log('■ 同じ容器が2枚の指図書に載っていても二重に引かない');
{
  /* ★ 索引は全部の行から範囲を持つようにした（26-60619 は2枚ある）。
       同じ容器が2枚に載っていると、足し合わせる数え方では2回引いてしまう。 */
  const s = build([lotRow({ id: 'L1', a: '1', b: '100', 本数: 100, 状態: '入庫済',
                            置場番号: 7, 置場名: 'x' })],
    [loc({ a50: 100 })], [
      { no: 'A', prefix: 'HEP', a: 1, b: 60, date: '2026-09-14' },
      { no: 'A', prefix: 'HEP', a: 41, b: 80, date: '2026-09-15' },   // 41〜60 が重なる
    ]);
  const r = s.matchProdLotsWithOrders();
  chk('★重なる範囲は1回だけ数える（1〜80 の80本。60+40=100 ではない）',
    r.引いた本数 === 80, r);
  chk('置場からも80本だけ引く', s.__locs[0].a50 === 20, s.__locs[0]);

  // 隣り合う範囲はつながって数える
  const s2 = build([lotRow({ id: 'L2', a: '1', b: '100', 本数: 100, 状態: '入庫済',
                             置場番号: 7, 置場名: 'x' })],
    [loc({ a50: 100 })], [
      { no: 'B', prefix: 'HEP', a: 1, b: 50, date: '2026-09-14' },
      { no: 'B', prefix: 'HEP', a: 51, b: 100, date: '2026-09-15' },
    ]);
  chk('隣り合う2枚は合わせて100本', s2.matchProdLotsWithOrders().引いた本数 === 100);
}
