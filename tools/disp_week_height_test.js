...
const REPLY = {
  ...
  getArmMonthlyData: () => ({ updated: 'x', error: null, months: [], kinds: [], total: 0,
    startMonth: null, today: null, prev: null, sourceName: null, harvestedAt: null }),
...
(async () => {
  ...
  /* 2026/09/11 に「この週の空きトラック」の欄を足したぶん 893→951px
     （数えなかった内訳を出すので2行になる）。表そのもの（653px）は
     変わっていない。ここは「際限なく伸びていないか」を見る枠。 */
  chk('カード全体が1000px以下になっている', closed.カード全体 <= 1000, closed);
